...
if cfg.DATASET.DATASET_NAME == 'VideoDeblur':
    ...
    __C.DIR.DATASET_ROOT = './datasets/data/DeepVideoDeblurringDataset/'
...
if cfg.DATASET.DATASET_NAME == 'VideoDeblurReal':
    ...
    __C.DIR.DATASET_ROOT = './datasets/data/qualitative_datasets/'
    __C.DIR.IMAGE_BLUR_PATH = os.path.join(__C.DIR.DATASET_ROOT,'%s/%s/input/%s.jpg')
...
__C.DATA.STD                            = [255.0]
__C.DATA.MEAN                           = [0.0]
__C.DATA.CROP_IMG_SIZE                  = [256,256]              # Crop image size: height, width,train(128)
...
__C.TRAIN.LR_MILESTONES                 = [5,80,160,250]
