...
class RandomColorChannel(object):
    def __call__(self, seq_blur, seq_clear):
        random_order = np.random.permutation(3)  #对0,1,2的随机排列，对通道的随机排列
...
class CenterCrop(object):   #每次都是中间的大小为crop_size的部分
...
class RandomCrop(object):  #每次都是随机的，裁剪出的图片大小为crop_size
